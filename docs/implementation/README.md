...
MVP 固有の M1 から M10 の詳細は `mvp/phases.md` で管理します。
...
- `mvp/phases.md`: MVP の M1 から M10 までのフェーズ定義と完了条件を管理する
...
- 完了通知は Bootstrap Toast、入力エラーや画面内に残す警告は alert として扱います。
- agent task Markdown の `実装時の記録` には、テンプレ文を残さず、実際の `実装方針` / `変更ファイル` / `実装結果` / `確認結果` / `残課題` を記録します。
- MVP の Service 状態遷移では、事前取得と業務チェックを正とします。更新件数による競合対策を入れる場合は、実装前に必要性を説明し、ユーザー確認を挟みます。
