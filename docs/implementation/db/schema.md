...
会社を表します。
会社削除は物理削除せず、`is_deleted = TRUE` の論理削除として扱います。
削除済み会社コードは再利用しません。
会社を論理削除しても、部署、ユーザー、申請、資産、会社別マスタ値などの配下データは物理削除しません。
配下データが存在していても、管理画面で警告表示したうえで会社の論理削除は可能とします。
削除済み会社は通常の新規作成・選択導線には表示せず、管理画面では削除済みであることを区別して表示します。
