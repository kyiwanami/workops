...
public record RequestForm(
    @NotNull(message = "申請種別を選択してください。") Long requestTypeValueId,
    Long assetId,
    @NotBlank(message = "件名を入力してください。") @Size(max = 100, message = "件名は100文字以内で入力してください。")
        String title,
    @Size(max = 2000, message = "申請内容は2000文字以内で入力してください。") String content) {
