 ...
 *
 * <p>資産分類は {@code generic_master_values} の会社別値として管理する。削除は物理削除ではなく {@code is_deleted = TRUE}
...
public class AssetCategoryMasterService {
  ...
  public List<AssetCategoryMasterListItem> findList(
    ...
    return assetCategoryMasterMapper.findListByCompanyIdAndSearchForm(
        currentUser.companyId(), assetCategoryMasterSearchForm);
  }
  ...
  public AssetCategoryMasterDetail findForEdit(Long id) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    return assetCategoryMasterMapper
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.INTERNAL_SERVER_ERROR, "資産分類マスタ種別が見つかりません。"));
  }

  private AssetCategoryMasterDetail findActiveForOperation(
  ...
  private AssetCategoryMasterDetail findDeletedForOperation(
      Long id, LoginUserContext currentUser, String operation) {
    return assetCategoryMasterMapper
  ...
  private void assertUniqueCodeForCreate(LoginUserContext currentUser, String code) {
    if (assetCategoryMasterMapper.existsCodeByCompanyId(currentUser.companyId(), code)) {
      logRejected(
          currentUser, OPERATION_ASSET_CATEGORY_CREATE, null, REASON_DUPLICATE_MASTER_VALUE_CODE);
