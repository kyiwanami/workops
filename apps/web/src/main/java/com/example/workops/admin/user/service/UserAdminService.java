 ...
 * のみ、TENANTユーザーは会社に所属しTENANT系権限だけを割り当てられる。
 */
@Service
public class UserAdminService {
  ...
  public UserDetail findTenantUserDetail(Long userId) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    return userAdminMapper
        .findTenantUserByIdAndCompanyId(userId, currentUser.companyId())
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "ユーザーが見つかりません。"));
  }
   ...
   *
   * <p>Cognitoユーザー作成境界を呼び出し、返却された {@code cognito_sub} をWorkOps DBへ登録する。
   *
  ...
  public Long createInitialTenantManager(
      Long companyId, String username, String name, String email) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
   ...
   *
   * <p>{@code actor_type}、{@code company_id}、{@code cognito_sub} は変更しない。
   *
   * @param userId ユーザーID
   ...
   *
   * <p>最終UPDATEでも現在ユーザーの会社IDをSQL条件に含め、他社ユーザーやPLATFORMユーザーは更新しない。
   *
   * @param userId ユーザーID
   * @param userEditForm 更新後のユーザー編集フォーム
   * @throws ResponseStatusException 他社ユーザー、PLATFORMユーザー、権限セット不整合、email重複、 TENANT_MANAGER 0人化が発生する場合
   */
  @Transactional
  @PreAuthorize("hasAuthority('TENANT_MANAGER')")
  ...
  private Long createUser(
    ...
    ProvisionedCognitoUser provisionedCognitoUser =
        cognitoUserProvisioner.provision(new CognitoUserProvisionRequest(username, email));
  ...
  private UserEditTarget requirePlatformUserEditTarget(Long userId) {
    return userAdminMapper
        .findPlatformUserEditTarget(userId)
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "ユーザーが見つかりません。"));
  }
  ...
  private UserEditTarget requireTenantUserEditTarget(Long userId) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    return userAdminMapper
        .findTenantUserEditTargetByIdAndCompanyId(userId, currentUser.companyId())
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "ユーザーが見つかりません。"));
  }
  ...
  private void updateUser(
    ...
    List<String> permissionSetCodes =
        normalizePermissionSetCodes(userEditForm.permissionSetCodes());
  ...
  private Long resolveDepartmentId(Long companyId, Long departmentId) {
    ...
    return userAdminMapper
        .findActiveDepartmentIdByCompanyId(departmentId, companyId)
  ...
  private void assertPermissionSets(String actorType, List<String> permissionSetCodes) {
    ...
    if (ACTOR_TYPE_PLATFORM.equals(actorType)
        && !PermissionSetCode.isValidPlatformCodes(permissionSetCodes)) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "PLATFORMユーザーの権限セットが不正です。");
    }
    if (ACTOR_TYPE_TENANT.equals(actorType)
        && !PermissionSetCode.isValidTenantCodes(permissionSetCodes)) {
  ...
  private List<String> normalizePermissionSetCodes(List<String> permissionSetCodes) {
    ...
    return permissionSetCodes.stream().distinct().toList();
  }
}
