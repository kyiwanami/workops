...
public class DbLoginUserContextFactory {
  ...
  private static final String REASON_CODE_PERMISSION_SET_NOT_ASSIGNED =
  ...
  public DbLoginUserContextFactory(
      UserAccountMapper userAccountMapper, SecurityEventLogger securityEventLogger) {
  ...
  public Optional<LoginUserContext> fromCognitoSub(String cognitoSub) {
    ...
    return userAccountMapper.findByCognitoSub(cognitoSub).map(this::toLoginUserContext);
  }

  private LoginUserContext toLoginUserContext(UserAccountRow row) {
    List<PermissionSetContext> permissionSets =
        userAccountMapper.findPermissionSetsByUserId(row.userId()).stream()
  ...
  private void assertValidLoginUser(UserAccountRow row, List<PermissionSetContext> permissionSets) {
    ...
    List<String> permissionSetCodes =
        permissionSets.stream().map(PermissionSetContext::code).toList();
  ...
  private void logInvalidLoginUser(
      UserAccountRow row, List<PermissionSetContext> permissionSets, String reasonCode) {
