 ...
 *
 * <p>認証主体の外部IDやメールアドレスは出さず、requestId、イベント種別、理由コード、 DB突合後のWorkOps利用者IDを中心にCloudWatch
 * Logsで調査できる値だけを出力する。
 */
@Component
public class SecurityEventLogger {
  ...
  private static final Logger SECURITY_LOGGER =
      LoggerFactory.getLogger("com.example.workops.security");
  ...
  private String format(SecurityEventLogRecord record, String result) {
    ...
    return "requestId="
        ...
        + " userId="
        + userId(loginUserContext)
        + " companyId="
        + companyId(loginUserContext)
        + " actorType="
        + actorType(loginUserContext)
        + " authorities="
