...
public class CognitoAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(
    ...
    if (!(authentication.getPrincipal() instanceof OidcUser oidcUser)) {
      securityEventLogger.logRejected(
          new SecurityEventLogRecord(
              null, EVENT_TYPE_AUTHENTICATION_REJECTED, REASON_CODE_OIDC_PRINCIPAL_MISSING, null));
      response.sendError(HttpStatus.FORBIDDEN.value());
      return;
    }

    LoginUserContext loginUserContext =
        dbLoginUserContextFactory.fromCognitoSub(oidcUser.getSubject()).orElse(null);
    if (loginUserContext == null) {
      securityEventLogger.logRejected(
          new SecurityEventLogRecord(
              null, EVENT_TYPE_AUTHENTICATION_REJECTED, REASON_CODE_USER_NOT_LINKED, null));
      response.sendError(HttpStatus.FORBIDDEN.value());
      return;
    }
    ...
    securityEventLogger.logSuccess(
        new SecurityEventLogRecord(
            loginUserContext, EVENT_TYPE_AUTHENTICATION_SUCCEEDED, null, null));
  ...
  private LoginRouteActorType extractExpectedActorType(Authentication authentication) {
    if (authentication instanceof OAuth2AuthenticationToken oauth2Authentication) {
      return LoginRouteActorType.fromRegistrationId(
