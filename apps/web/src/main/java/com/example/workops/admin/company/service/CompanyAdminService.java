 ...
 *
 * <p>会社コードは削除済み会社も含めて全体一意とし、会社削除は物理削除ではなく
 * {@code companies.is_deleted = TRUE} の論理削除として扱う。会社作成時は会社別初期マスタ値と
 * 初期 {@code TENANT_MANAGER} を同一トランザクションで作成する。</p>
 */
@Service
public class CompanyAdminService {
     ...
     * 未削除会社を論理削除する。
     *
     * <p>部署、ユーザー、申請、資産、会社別マスタ値などの配下データは物理削除しない。</p>
     *
     * @param companyId 会社ID
     * @throws ResponseStatusException 対象会社が存在しない、または削除済みの場合
     */
