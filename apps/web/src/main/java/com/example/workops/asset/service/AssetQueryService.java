 ...
 *
 * <p>参照系の会社境界は、現在ユーザーの {@code companyId} をMapper条件へ渡して守る。 {@code can...}
 * メソッドは画面表示用の補助判定であり、更新系の最終判定は {@link AssetCommandService} 側で再度実施する。
 */
@Service
public class AssetQueryService {
  ...
  public boolean canCreateAsset() {
    return currentUserProvider.currentUser().map(this::hasAssetWritePermission).orElse(false);
  }
  ...
  public boolean canDeleteAsset(AssetDetail assetDetail) {
    return currentUserProvider.currentUser().map(this::hasAssetDeletePermission).orElse(false);
  }
  ...
  private boolean hasAssetWritePermission(LoginUserContext currentUser) {
    return currentUser.permissionSets().stream()
        .anyMatch(
            permissionSet ->
                PermissionSetCode.TENANT_EDITOR.name().equals(permissionSet.code())
  ...
  private boolean hasAssetDeletePermission(LoginUserContext currentUser) {
    return currentUser.permissionSets().stream()
        .anyMatch(
            permissionSet -> PermissionSetCode.TENANT_MANAGER.name().equals(permissionSet.code()));
  }
}
