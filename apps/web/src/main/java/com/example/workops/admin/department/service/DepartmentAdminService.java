 ...
 *
 * <p>部署コードは削除済み部署も含めて会社内一意とし、部署削除は物理削除ではなく {@code departments.is_deleted = TRUE}
 * の論理削除として扱う。TENANT_MANAGER導線では、 現在ユーザーの会社だけを対象にする。
 */
@Service
public class DepartmentAdminService {
  ...
  private static final String REASON_DEPARTMENT_NOT_FOUND_OR_FORBIDDEN =
      "DEPARTMENT_NOT_FOUND_OR_FORBIDDEN";
  ...
  public DepartmentListPage findPlatformDepartmentList(
      Long companyId, DepartmentSearchForm departmentSearchForm) {
  ...
  @Transactional
  @PreAuthorize("hasAuthority('PLATFORM_ADMIN')")
  public void updatePlatformDepartment(
      Long companyId, Long departmentId, DepartmentForm departmentForm) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    Long activeCompanyId = findActiveCompanyId(companyId);
  ...
  private Long createDepartment(
      Long companyId, DepartmentForm departmentForm, LoginUserContext currentUser) {
  ...
  private void updateDepartment(
    ...
    departmentAdminMapper.updateActiveDepartmentNameByIdAndCompanyId(
        departmentId, companyId, departmentForm.name(), currentUser.userId());
  ...
  private void deleteDepartment(Long companyId, Long departmentId, LoginUserContext currentUser) {
    ...
    departmentAdminMapper.logicalDeleteActiveDepartmentByIdAndCompanyId(
        departmentId, companyId, currentUser.userId());
    logSuccess(currentUser, OPERATION_DEPARTMENT_DELETE, departmentId);
  }

  private DepartmentListPage findDepartmentListPage(
      Long companyId, DepartmentSearchForm departmentSearchForm, boolean includeDepartments) {
    Long activeCompanyId = findActiveCompanyId(companyId);
    String companyCode =
        departmentAdminMapper
            .findActiveCompanyCodeById(activeCompanyId)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "会社が見つかりません。"));
    String companyName =
        departmentAdminMapper
            .findActiveCompanyNameById(activeCompanyId)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "会社が見つかりません。"));
    List<DepartmentListItem> departments = List.of();
    if (includeDepartments) {
      departments =
          departmentAdminMapper.findDepartmentsByCompanyIdAndSearchForm(
              activeCompanyId, departmentSearchForm);
    }
    return new DepartmentListPage(
        activeCompanyId, companyCode, companyName, departmentSearchForm.showDeleted(), departments);
  }

  private Long findActiveCompanyId(Long companyId) {
    return departmentAdminMapper
        .findActiveCompanyId(companyId)
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "会社が見つかりません。"));
  }

  private DepartmentListItem findActiveDepartment(
      Long companyId, Long departmentId, LoginUserContext currentUser, String operation) {
    return departmentAdminMapper
        .findActiveDepartmentByIdAndCompanyId(departmentId, companyId)
        .orElseThrow(
            () -> {
              if (currentUser != null) {
                logRejected(
                    currentUser, operation, departmentId, REASON_DEPARTMENT_NOT_FOUND_OR_FORBIDDEN);
              }
              return new ResponseStatusException(HttpStatus.NOT_FOUND, "部署が見つかりません。");
            });
  }

  private void assertUniqueDepartmentCode(
      Long companyId, String code, LoginUserContext currentUser) {
  ...
  private void logSuccess(LoginUserContext currentUser, String operation, Long departmentId) {
    operationLogger.logSuccess(
        new OperationLogRecord(
            currentUser, operation, DEPARTMENT_TARGET_TYPE, departmentId, null, false, null));
  }

  private void logRejected(
      LoginUserContext currentUser, String operation, Long departmentId, String reasonCode) {
