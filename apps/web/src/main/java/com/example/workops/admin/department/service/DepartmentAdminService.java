...
/**
 * PLATFORM_ADMINとTENANT_MANAGER向け部署一覧・作成ユースケースを扱うService。
 */
@Service
public class DepartmentAdminService {
    ...
    public DepartmentListPage findPlatformDepartmentList(Long companyId) {
        return findDepartmentListPage(companyId, true);
    }

    @Transactional(readOnly = true)
    @PreAuthorize("hasAuthority('TENANT_MANAGER')")
    public DepartmentListPage findTenantDepartmentList() {
        LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
        return findDepartmentListPage(currentUser.companyId(), true);
    }

    @Transactional(readOnly = true)
    @PreAuthorize("hasAuthority('PLATFORM_ADMIN')")
    public DepartmentListPage findPlatformDepartmentFormPage(Long companyId) {
        return findDepartmentListPage(companyId, false);
    }
    ...
    public DepartmentListPage findTenantDepartmentFormPage() {
        LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
        return findDepartmentListPage(currentUser.companyId(), false);
    }
    ...
    private Long createDepartment(Long companyId, DepartmentForm departmentForm, LoginUserContext currentUser) {
        ...
        logSuccess(currentUser, departmentId);
        return departmentId;
    }

    private DepartmentListPage findDepartmentListPage(Long companyId, boolean includeDepartments) {
        Long activeCompanyId = findActiveCompanyId(companyId);
        ...
        if (includeDepartments) {
            departments = departmentAdminMapper.findActiveDepartmentsByCompanyId(activeCompanyId);
        }
        return new DepartmentListPage(activeCompanyId, companyCode, companyName, departments);
    }
    ...
    private void logSuccess(LoginUserContext currentUser, Long departmentId) {
        operationLogger.logSuccess(new OperationLogRecord(
                currentUser,
                OPERATION_DEPARTMENT_CREATE,
    ...
    private void logRejected(LoginUserContext currentUser) {
        operationLogger.logRejected(new OperationLogRecord(
                ...
                DEPARTMENT_TARGET_TYPE,
                null,
                REASON_DUPLICATE_DEPARTMENT_CODE,
                false,
