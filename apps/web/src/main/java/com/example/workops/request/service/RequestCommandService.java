 ...
 * 申請ステータス条件、会社境界、申請種別と資産の選択可否を検証する。会社境界はMapperの {@code company_id} 条件で守り、主要な成功・拒否結果は業務操作ログへ出力する。
 */
@Service
public class RequestCommandService {
  ...
  public Long createDraft(RequestForm requestForm) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    assertSelectableRequestType(
        requestForm.requestTypeValueId(), currentUser, OPERATION_REQUEST_CREATE, null);
  ...
  public void updateDraft(Long id, RequestForm requestForm) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    RequestDetail requestDetail =
        findRequestForOperation(id, currentUser, OPERATION_REQUEST_UPDATE);
    assertEditableDraft(requestDetail, currentUser, OPERATION_REQUEST_UPDATE);
    assertSelectableRequestType(
        requestForm.requestTypeValueId(), currentUser, OPERATION_REQUEST_UPDATE, id);
  ...
  public void submitDraft(Long id) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    RequestDetail requestDetail =
        findRequestForOperation(id, currentUser, OPERATION_REQUEST_SUBMIT);
  ...
  public void withdrawSubmitted(Long id) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    RequestDetail requestDetail =
        findRequestForOperation(id, currentUser, OPERATION_REQUEST_WITHDRAW);
    ...
    requestMapper.withdrawSubmittedByIdAndCompanyId(
        id, currentUser.companyId(), currentUser.userId(), currentUser.userId());
  ...
  public void approveSubmitted(Long id) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    RequestDetail requestDetail =
        findRequestForOperation(id, currentUser, OPERATION_REQUEST_APPROVE);
  ...
  public void rejectSubmitted(Long id, RequestReviewForm requestReviewForm) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    RequestDetail requestDetail =
        findRequestForOperation(id, currentUser, OPERATION_REQUEST_REJECT);
    ...
    requestMapper.rejectSubmittedByIdAndCompanyId(
        id, currentUser.companyId(), requestReviewForm.reviewComment(), currentUser.userId());
  ...
  public void remandSubmitted(Long id, RequestReviewForm requestReviewForm) {
    LoginUserContext currentUser = currentUserProvider.requireCurrentUser();
    RequestDetail requestDetail =
        findRequestForOperation(id, currentUser, OPERATION_REQUEST_REMAND);
  ...
  private RequestDetail findRequestForOperation(
  ...
  private void assertEditableDraftForRead(
      RequestDetail requestDetail, LoginUserContext currentUser) {
  ...
  private void assertEditableDraft(
      RequestDetail requestDetail, LoginUserContext currentUser, String operation) {
    if (!Objects.equals(requestDetail.requesterUserId(), currentUser.userId())) {
      logRejected(
          currentUser,
          operation,
  ...
  private void assertSubmittableDraft(
      RequestDetail requestDetail, LoginUserContext currentUser, String operation) {
    if (!Objects.equals(requestDetail.requesterUserId(), currentUser.userId())) {
      logRejected(
          currentUser,
  ...
  private void assertWithdrawableSubmitted(
      RequestDetail requestDetail, LoginUserContext currentUser, String operation) {
    if (!Objects.equals(requestDetail.requesterUserId(), currentUser.userId())) {
      logRejected(
          currentUser,
  ...
  private void assertApprovableSubmitted(
      RequestDetail requestDetail, LoginUserContext currentUser, String operation) {
    if (!SUBMITTED_STATUS_CODE.equals(requestDetail.statusCode())) {
      logRejected(
          currentUser,
  ...
  private void assertRejectableSubmitted(
      RequestDetail requestDetail, LoginUserContext currentUser, String operation) {
    if (!SUBMITTED_STATUS_CODE.equals(requestDetail.statusCode())) {
      logRejected(
          currentUser,
  ...
  private void assertRemandableSubmitted(
      RequestDetail requestDetail, LoginUserContext currentUser, String operation) {
    if (!SUBMITTED_STATUS_CODE.equals(requestDetail.statusCode())) {
      logRejected(
          currentUser,
  ...
  private void assertSelectableRequestType(
      Long requestTypeValueId, LoginUserContext currentUser, String operation, Long targetId) {
    if (!requestMapper.existsRequestTypeByIdAndCompanyId(
        requestTypeValueId, currentUser.companyId())) {
      logRejected(
          currentUser,
          operation,
          targetId,
          REASON_INVALID_REQUEST_TYPE,
          false,
          EXCEPTION_RESPONSE_STATUS);
  ...
  private void assertSelectableAsset(
      Long assetId, LoginUserContext currentUser, String operation, Long targetId) {
    if (assetId != null
        && !requestMapper.existsSelectableAssetByIdAndCompanyId(assetId, currentUser.companyId())) {
      logRejected(
          currentUser, operation, targetId, REASON_INVALID_ASSET, false, EXCEPTION_RESPONSE_STATUS);
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "資産が不正です。");
    }
  }

  private void logSuccess(
      LoginUserContext currentUser, String operation, Long targetId, boolean reasonCommentPresent) {
  ...
  private boolean hasReviewComment(RequestReviewForm requestReviewForm) {
    return requestReviewForm.reviewComment() != null
        && !requestReviewForm.reviewComment().isEmpty();
  }
}
