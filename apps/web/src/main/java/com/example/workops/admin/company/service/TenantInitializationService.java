 ...
 *
 * <p>このServiceは会社作成トランザクション内で呼び出され、申請種別と資産分類の初期値を 会社別の {@code generic_master_values} として投入する。{@code
 * generic_master} の種別自体は 複製しない。
 */
@Service
public class TenantInitializationService {
  ...
  private static final List<TenantInitialMasterValue> INITIAL_VALUES =
      List.of(
          ...
          new TenantInitialMasterValue(
              GENERIC_MASTER_ASSET_CATEGORY, "NETWORK_DEVICE", "ネットワーク機器", 50),
          new TenantInitialMasterValue(GENERIC_MASTER_ASSET_CATEGORY, "OTHER", "その他", 60),
          new TenantInitialMasterValue(
              GENERIC_MASTER_REQUEST_TYPE, "EQUIPMENT_PURCHASE", "備品購入申請", 10),
          new TenantInitialMasterValue(GENERIC_MASTER_REQUEST_TYPE, "REPAIR_REQUEST", "修理依頼申請", 20),
          new TenantInitialMasterValue(
              GENERIC_MASTER_REQUEST_TYPE, "DISPOSAL_REQUEST", "廃棄申請", 30));
  ...
  public void initializeTenant(Long companyId, Long currentUserId) {
    ...
    for (TenantInitialMasterValue initialValue : INITIAL_VALUES) {
      Long genericMasterId =
          resolveGenericMasterId(
              initialValue.genericMasterCode(), assetCategoryMasterId, requestTypeMasterId);
  ...
  private Long findRequiredGenericMasterId(String code) {
    return companyAdminMapper
        .findActiveGenericMasterIdByCode(code)
        .orElseThrow(
            () ->
                new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "汎用マスタ種別が見つかりません。"));
  }

  private Long resolveGenericMasterId(
      String genericMasterCode, Long assetCategoryMasterId, Long requestTypeMasterId) {
