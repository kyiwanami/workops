...
class SecurityAuthorizationEventListenerTests {
  ...
  void authorizationDeniedEventOutputsLoginUserContext(CapturedOutput output) {
    SecurityAuthorizationEventListener listener =
        new SecurityAuthorizationEventListener(new SecurityEventLogger());
    Authentication authentication =
        new UsernamePasswordAuthenticationToken(
            loginUserContext(), null, List.of(new SimpleGrantedAuthority("TENANT_MANAGER")));

    listener.onAuthorizationDenied(
        new AuthorizationDeniedEvent<>(
            () -> authentication, new Object(), new AuthorizationDecision(false)));
  ...
  void authorizationDeniedEventWithoutLoginUserContextOutputsHyphen(CapturedOutput output) {
    ...
    Authentication authentication =
        new UsernamePasswordAuthenticationToken(
            "principal-name", null, List.of(new SimpleGrantedAuthority("TENANT_MANAGER")));
