...
class CognitoLogoutSuccessHandlerTests {
  ...
  private static final String HOSTED_UI_DOMAIN_BASE_URL =
  ...
  void unknownActorTypeReturnsLogin() throws Exception {
    ...
    handler.onLogoutSuccess(
        new MockHttpServletRequest(), response, authentication(unknownActorTypeUser()));
  ...
  void missingCognitoSettingReturnsLogin() throws Exception {
    CognitoLogoutSuccessHandler handler =
        new CognitoLogoutSuccessHandler("", LOGOUT_URI, PLATFORM_CLIENT_ID, TENANT_CLIENT_ID);
    MockHttpServletResponse response = new MockHttpServletResponse();
  ...
  private CognitoLogoutSuccessHandler handler() {
    return new CognitoLogoutSuccessHandler(
        HOSTED_UI_DOMAIN_BASE_URL, LOGOUT_URI, PLATFORM_CLIENT_ID, TENANT_CLIENT_ID);
  }
