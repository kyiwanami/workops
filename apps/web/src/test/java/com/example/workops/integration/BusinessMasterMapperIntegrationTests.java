...
class BusinessMasterMapperIntegrationTests extends MapperIntegrationTestBase {
  ...
  void requestTypeMasterMapperHandlesCrudAndCompanyBoundary() {
    ...
    requestTypeMasterMapper.insertRequestType(
        masterId, 1L, "M8_REQUEST_TYPE", "M8申請種別", 90, 3L, 3L);
    ...
    int otherCompanyUpdated =
        requestTypeMasterMapper.updateActiveByIdAndCompanyId(requestTypeId, 2L, "他社更新", 91, 3L);
    int updated =
        requestTypeMasterMapper.updateActiveByIdAndCompanyId(requestTypeId, 1L, "M8申請種別更新", 91, 3L);
    ...
    assertThat(
            requestTypeMasterMapper.findListByCompanyIdAndSearchForm(
                1L, new RequestTypeMasterSearchForm(false)))
        .noneSatisfy(item -> assertThat(item.id()).isEqualTo(requestTypeId));
    assertThat(
            requestTypeMasterMapper.findListByCompanyIdAndSearchForm(
                1L, new RequestTypeMasterSearchForm(true)))
  ...
  void assetCategoryMasterMapperHandlesCrudAndCompanyBoundary() {
    ...
    assetCategoryMasterMapper.insertAssetCategory(
        masterId, 1L, "M8_ASSET_CATEGORY", "M8資産分類", 90, 3L, 3L);
    ...
    assertThat(assetCategoryMasterMapper.findActiveByIdAndCompanyId(assetCategoryId, 1L))
        .isPresent();
    assertThat(assetCategoryMasterMapper.findActiveByIdAndCompanyId(assetCategoryId, 2L)).isEmpty();

    int otherCompanyDeleted =
        assetCategoryMasterMapper.logicalDeleteActiveByIdAndCompanyId(assetCategoryId, 2L, 3L);
    int deleted =
        assetCategoryMasterMapper.logicalDeleteActiveByIdAndCompanyId(assetCategoryId, 1L, 3L);
    ...
    assertThat(assetCategoryMasterMapper.findDeletedByIdAndCompanyId(assetCategoryId, 1L))
        .isPresent();
    assertThat(
            assetCategoryMasterMapper.findListByCompanyIdAndSearchForm(
                1L, new AssetCategoryMasterSearchForm(false)))
        .noneSatisfy(item -> assertThat(item.id()).isEqualTo(assetCategoryId));
    assertThat(
            assetCategoryMasterMapper.findListByCompanyIdAndSearchForm(
                1L, new AssetCategoryMasterSearchForm(true)))
