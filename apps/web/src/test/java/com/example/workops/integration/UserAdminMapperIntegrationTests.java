...
class UserAdminMapperIntegrationTests extends MapperIntegrationTestBase {
  ...
  void userDisplayIncludesPermissionSets() {
    assertThat(userAdminMapper.findPlatformUserById(3L))
        .hasValueSatisfying(
            user -> assertThat(user.permissionSetDisplay()).isEqualTo("TENANT_MANAGER / 管理者"));
  }
  ...
  void updateUserEditableFieldsAndReplacePermissionSets() {
    ...
    assertThat(
            userAdminMapper.updateTenantUserEditableFields(
                userId, 2L, "編集後", "edit-target-updated@example.local", 2L, 7L))
        .isZero();

    userAdminMapper.updateTenantUserEditableFields(
        userId, 1L, "編集後", "edit-target-updated@example.local", 2L, 7L);
  ...
  void emailDuplicateExcludingUserAndTenantManagerCountWork() {
    assertThat(userAdminMapper.existsTenantEmailExcludingUser(1L, 3L, "kthm-manager@example.local"))
        .isFalse();
    assertThat(userAdminMapper.existsTenantEmailExcludingUser(1L, 3L, "kthm-editor@example.local"))
        .isTrue();
    assertThat(userAdminMapper.existsPlatformEmailExcludingUser(7L, "platform-admin@example.local"))
        .isFalse();
