...
class HomeControllerWebTests {
    ...
    @MockitoBean
    private ApplicationExceptionLogger applicationExceptionLogger;

    @Test
    void platformAdminHomeRendersOnlyPlatformLinks() throws Exception {
        signIn("PLATFORM_ADMIN");

        mockMvc.perform(homeRequest())
    ...
    private MockHttpServletRequestBuilder homeRequest() {
        return get("/").requestAttr("_csrf", new DefaultCsrfToken("X-CSRF-TOKEN", "_csrf", "test-token"));
    }
