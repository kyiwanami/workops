...
class ApplicationExceptionLoggerTests {
    ...
    private final ApplicationExceptionLogger applicationExceptionLogger = new ApplicationExceptionLogger();
    ...
    void logOutputsKeyValueMessageAndStackTrace(CapturedOutput output) {
        ...
        applicationExceptionLogger.log(new ApplicationExceptionLogRecord(
                "APPLICATION_EXCEPTION",
                500,
                "/sample/path",
                "IllegalStateException"), new IllegalStateException("body-value"));

        String logs = output.toString();
        assertThat(logs).contains("ERROR");
        assertThat(logs).contains("com.example.workops.application");
        assertThat(applicationExceptionLine(logs)).contains(
    ...
    void emptyOptionalValuesAreLoggedAsHyphen(CapturedOutput output) {
        ...
        assertThat(applicationExceptionLine(output.toString())).contains(
                "requestId=- eventType=- status=500 path=- exceptionType=-");
    }
    ...
    void keyValueLineDoesNotContainExceptionMessage(CapturedOutput output) {
        applicationExceptionLogger.log(new ApplicationExceptionLogRecord(
                ...
                500,
                "/sample/path",
                "IllegalStateException"), new IllegalStateException("token=secret-value"));
    ...
    void applicationExceptionLogRecordDoesNotHaveSensitiveFields() {
        List<String> componentNames = Arrays.stream(ApplicationExceptionLogRecord.class.getRecordComponents())
    ...
    private String applicationExceptionLine(String logs) {
        return logs.lines()
                .filter(line -> line.contains("eventType=APPLICATION_EXCEPTION") || line.contains("eventType=-"))
                .findFirst()
                .orElse("");
    }
}
